...
const FavoritesScreen = () => {
  ...
  useEffect(() => {
    if (!user?.id) return; // Don't fetch if user.id is not ready
    console.log("Favorites fetch user.id:", user.id);
    console.log("Fetching favorites from:", `${API_BASE_URL}/favorites/${user.id}`);
    const loadFavorites = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/favorites/${user.id}`);
